...
#addprocs(8)
cd("/nfs/sloanlab001/projects/edema-partners_proj/Trees/Outputs/acuity")

function RunOCT(technique, maxDepth, minBucket, seed, test_X, test_y, train_X, valid_X, train_y, valid_y)	

	lnr = OptimalTrees.OptimalTreeClassifier(fast_num_support_restarts = 1000, ls_num_tree_restarts = 1000, ls_random_seed = seed)
	...
	cd("/nfs/sloanlab001/projects/edema-partners_proj/Trees/Outputs/acuity")
        OptimalTrees.writedot("OCT_auc_cluster_1000_$(seed)_$(technique).dot", grid.lnr)
        run(`dot -Tpng -o OCT_auc_cluster_1000_$(seed)_$(technique).png OCT_auc_cluster_1000_$(seed)_$(technique).dot`)
	OptimalTrees.writejson("OCT_auc_cluster_1000_$(seed)_$(technique).json", grid.lnr; write_tree = true)
	...
	println("The cross-validated OCT misclassification rate is ", misclass)
	println("The cross-validated OCT auc is ", auc)
...
for technique in techniques
	df_b = DataFrame()
	for seed in 1:5
	    	cd("/nfs/sloanlab001/projects/edema-partners_proj/clean_data/acuity")
		...
		pred_y, auc = RunOCT(technique, maxDepth, minBucket, seed, test_X[:,1:end], test_y[:,1],train_X[:,1:end],valid_X[:,1:end], train_y[:,1],valid_y[:,1])
	...
	println(technique)
	CSV.write("OCT_technique_auc_cluster_1000_$(technique).csv", df_b)
end
